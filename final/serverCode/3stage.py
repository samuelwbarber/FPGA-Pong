...
async def welcome_screen(websocket, path):
    async for message in websocket:
        ...
        if message.startswith("ENTER:"):
            ...
            if user not in clients:
                clients.append(user)
                if len(clients) == 1:
                    await websocket.send("1 0 Waiting") #Waiting for opponent
                elif len(clients) == 2:
                    user1 = clients[0]
                    await websocket.send("2 1 " + str(user1)) #Both players ready to play
...
start_server = websockets.serve(welcome_screen, "0.0.0.0", 6789)

asyncio.get_event_loop().run_until_complete(start_server)

playing = websockets.serve(echo, "0.0.0.0", 6789)

asyncio.get_event_loop().run_until_complete(playing)
